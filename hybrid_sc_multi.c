...
void main(int argc, char *argv[]){
    ...
    //printf("mkey:\n");
    BIGNUM *mkey_bn=BN_new();
    BN_rand(mkey_bn, 128, 0, 0);
    unsigned char *mkey = OPENSSL_malloc(128);
    int mkey_len = BN_bn2bin(mkey_bn, mkey);

    clock_t pmsg_start_time = clock();
    //printf("iv:\n");
    BIGNUM *iv_bn=BN_new();
    BN_rand(iv_bn, 128, 0, 0);
    unsigned char *iv = OPENSSL_malloc(128);
    ...
    unsigned char aad[] = "";
    size_t tag_len = 16;
    int pmsg_ciphertext_len = aead_encrypt_raw("aes-128-gcm", msg, msg_len, aad, strlen(aad), mkey, iv, iv_len, &ciphertext, &tag, tag_len);
    double pmsg_enc_time = (double)(clock() - pmsg_start_time) / CLOCKS_PER_SEC;
    size_t pmsg_enc_len = pmsg_ciphertext_len + iv_len;


    clock_t pmsg_dec_start_time = clock();
    unsigned char *pmsg_dec;
    size_t pmsg_dec_len = aead_decrypt_raw("aes-128-gcm",ciphertext, pmsg_ciphertext_len, aad, strlen(aad), tag, tag_len, mkey, iv, iv_len, &pmsg_dec);
    double pmsg_dec_time = (double)(clock() - pmsg_dec_start_time) / CLOCKS_PER_SEC;


    ec_keypair_t **xbs = malloc(num * sizeof(ec_keypair_t *));
    EVP_PKEY **rsaxbs = malloc(num * sizeof(EVP_PKEY *));
    EVP_PKEY **rsaybs = malloc(num * sizeof(EVP_PKEY *));
    ...
    for(int i =0; i<num; i++){
	    //printf("xb:\n");
	    xbs[i] = gen_ec_keypair(group_name);

	    //printf("rsa private:\n");
	    rsaxbs[i] = EVP_RSA_gen(1024);
	    //    PEM_write_PrivateKey(stdout,  rsa_xb, NULL, NULL, 0, NULL, NULL);

	    //printf("rsa public:\n");
	    rsaybs[i] = export_rsa_public_pkey(rsaxbs[i]);
	    //    PEM_write_PUBKEY(stdout,  rsa_yb);
    }

    double ecies_sig_enc_time =0;
    size_t ecies_sig_enc_len = 0;
    double ecies_sig_dec_time =0;
    size_t ecies_sig_dec_len = 0;

    double imessage_enc_time =0;
    size_t imessage_enc_len = 0;
    double imessage_dec_time =0;
    size_t imessage_dec_len = 0;

    double hpke_enc_time =0;
    size_t hpke_enc_len = 0;
    double hpke_dec_time =0;
    size_t hpke_dec_len = 0;
	
    double our_enc_time =0;
    size_t our_enc_len = 0;
    double our_dec_time =0;
    size_t our_dec_len = 0;

    for(int i =0; i<num; i++){
	    ec_keypair_t *ec_b = xbs[i];
	    EVP_PKEY *rsa_xb = rsaxbs[i];
	    EVP_PKEY *rsa_yb = rsaybs[i];

	    clock_t start_time = clock();
	    //printf("\necies_signature_enc_raw:\n");
	    ecies_signature_t *ec = ecies_signature_enc_raw(group_name, sig_name, ec_a->priv, digest_name, ec_b->pub, mkey, mkey_len);
	    ecies_sig_enc_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    ecies_sig_enc_len += ec->sig_len + ec->c->ye_len + ec->c->iv_len + ec->c->ciphertext_len;
	    ...
	    ecies_sig_dec_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    ecies_sig_dec_len +=msg_ec_len;
	    //printf("Default:  Done with  %f sd\n", elapsed_time2);
	    free(msg_ec);
	    free(ec->c->ye);
	    free(ec->c->ciphertext);
	    free(ec->c->iv);
	    free(ec->c);
	    free(ec->sig);
	    free(ec);
	    ...
	    start_time = clock();
	    //printf("\nhpke_auth_enc_raw:\n");
	    hpke_enc_t *hpke = hpke_auth_enc_raw(group_name, digest_name, ec_a, ec_b->pub_hex, mkey, mkey_len);
	     hpke_enc_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    //printf("Default:  Done with  %f sd\n", elapsed_time);
	    hpke_enc_len += hpke->tag_len + hpke->ye_len + hpke->ciphertext_len;
	    ...
	    hpke_dec_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    hpke_dec_len += msg_hpke_len;
	    //printf("Default:  Done with  %f sd\n", elapsed_time2);
	    free(msg_hpke);
	    free(hpke->ciphertext);
	    free(hpke->tag);
	    free(hpke->ye);
	    free(hpke);
	    ...
	    start_time = clock();
	    //printf("\nimessage_enc_raw\n");
	    imessage_t* imsg = imessage_enc_raw(group_name, digest_name, sig_name_sha1, ec_a, rsa_yb, mkey, mkey_len);
	    imessage_enc_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    //printf("Default:  Done with  %f sd\n", elapsed_time);


	    imessage_enc_len += imsg->iv_len + imsg->c1_len + imsg->c2_len + imsg->sig_len;
	    ...
	    imessage_dec_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    imessage_dec_len += msg_imsg_len;
	    //printf("Default:  Done with  %f sd\n", elapsed_time2);
	    free(msg_imsg);
	    free(imsg->c1);
	    free(imsg->c2);
	    ...
	    hybrid_signcryption_t* hybrid_sc = hybrid_signcryption_raw(group_name, digest_name, ec_a, ec_b->pub, 
			    ...
			    psk, strlen(psk), 
			    mkey, mkey_len);
	    our_enc_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    //printf("Default:  Done with  %f sd\n", elapsed_time);

	    our_enc_len += hybrid_sc->iv_len + hybrid_sc->ciphertext_len + hybrid_sc->tag_len + hybrid_sc->s_len;
	    ...
	    msg_hybrid_len = hybrid_unsigncryption_raw(group_name, digest_name, 
			    ...
			    psk, strlen(psk), 
			    &msg_hybrid);
	    our_dec_time += (double)(clock() - start_time) / CLOCKS_PER_SEC;
	    our_dec_len += msg_hybrid_len;
	    //printf("Default:  Done with  %f sd\n", elapsed_time2);
...
//	    BIO_printf(out_bio, "hybrid_signcryption,%d,%f,%f,%d,%d,%d,%d,%d,%d,%d\n", msg_len, elapsed_time, elapsed_time2, all_len, hybrid_sc->iv_len, hybrid_sc->ciphertext_len, hybrid_sc->tag_len, hybrid_sc->s_len, 0, 0 );
    ...
    BIO_printf(out_bio, "hybrid_signcryption,%d,%d,%d,%f,%d,%f,%d,%f,%d,%f\n", num, msg_len, pmsg_enc_len, pmsg_enc_time, pmsg_dec_len, pmsg_dec_time,our_enc_len, our_enc_time, our_dec_len, our_dec_time);
    BIO_printf(out_bio, "imessage,%d,%d,%d,%f,%d,%f,%d,%f,%d,%f\n", num, msg_len, pmsg_enc_len, pmsg_enc_time, pmsg_dec_len, pmsg_dec_time,imessage_enc_len, imessage_enc_time, imessage_dec_len, imessage_dec_time);
    BIO_printf(out_bio, "hpke,%d,%d,%d,%f,%d,%f,%d,%f,%d,%f\n", num, msg_len, pmsg_enc_len, pmsg_enc_time, pmsg_dec_len, pmsg_dec_time,hpke_enc_len, hpke_enc_time, hpke_dec_len, hpke_dec_time);
    BIO_printf(out_bio, "ecies_sig,%d,%d,%d,%f,%d,%f,%d,%f,%d,%f\n", num, msg_len, pmsg_enc_len, pmsg_enc_time, pmsg_dec_len, pmsg_dec_time,ecies_sig_enc_len, ecies_sig_enc_time, ecies_sig_dec_len, ecies_sig_dec_time);
